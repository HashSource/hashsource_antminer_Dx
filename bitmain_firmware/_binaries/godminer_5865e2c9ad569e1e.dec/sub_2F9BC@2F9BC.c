_DWORD *__fastcall sub_2F9BC(char *s2)
{
  _DWORD *result; // r0
  char v3[4100]; // [sp+10h] [bp-1004h] BYREF

  if ( dword_174240[0] > 0x13u )
  {
    V_LOCK();
    logfmt_raw(v3, 0x1000u, 0, "only support %d reject reasons", dword_174240[0]);
    V_UNLOCK();
    return (_DWORD *)zlog(
                       g_zc,
                       "/workspace/jenkins/jenkins/workspace/Antminer_D9_release/build/rootfs/buildroot/tmp/release/build"
                       "/godminer-origin_godminer-new/pool.c",
                       133,
                       "reject_reason_history_update",
                       28,
                       41,
                       20,
                       v3);
  }
  else
  {
    if ( !dword_174240[0] )
    {
      V_LOCK();
      logfmt_raw(v3, 0x1000u, 0, "reject reason init %d", dword_174240[0]);
      V_UNLOCK();
      zlog(
        g_zc,
        "/workspace/jenkins/jenkins/workspace/Antminer_D9_release/build/rootfs/buildroot/tmp/release/build/godminer-origi"
        "n_godminer-new/pool.c",
        133,
        "reject_reason_history_update",
        28,
        46,
        40,
        v3);
      reject_reason_map = 0;
      dword_18BB54 = 0;
      dword_18BB58 = 0;
      dword_18BB5C = 0;
      dword_18BB60 = 0;
    }
    result = (_DWORD *)map_get_(&reject_reason_map, s2);
    dword_18BB5C = (int)result;
    if ( result )
    {
      ++*result;
    }
    else
    {
      dword_18BB60 = 1;
      ++dword_174240[0];
      return (_DWORD *)map_set_((void **)&reject_reason_map, s2, &dword_18BB60, 4u);
    }
  }
  return result;
}

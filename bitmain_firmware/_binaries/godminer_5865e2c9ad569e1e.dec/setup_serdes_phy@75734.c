int __fastcall setup_serdes_phy(int a1, int a2)
{
  ...
  zlog(g_zc, v7, 170, "setup_serdes_phy", 16, 1057, 60, v15);
  V_LOCK();
  LOWORD(v8) = -10848;
  HIWORD(v8) = (unsigned int)"chip[%d] received dag size: 0x%llx, crc error: %s" >> 16;
  logfmt_raw((char *)v15, 0x1000u, 0, v8, "setup_serdes_phy", 1);
  V_UNLOCK();
  LOWORD(v9) = -12340;
  HIWORD(v9) = (unsigned int)"atus] asic %02x, reg %02x state %08x" >> 16;
  zlog(g_zc, v9, 170, "setup_serdes_phy", 16, 1064, 60, v15);
  v15[0] = 50462976;
  v15[1] = 117835012;
  v15[2] = 185207048;
  v16 = 252579084;

int psu_watchdog()
{
  ...
  V_LOCK();
  logfmt_raw(v16, 0x1000u, 0, 1342316, v1);
  V_UNLOCK();
  zlog(
    g_zc,
  ...
  {
    V_LOCK();
    logfmt_raw(v16, 0x1000u, 0, 1342348, "psu_watchdog");
    V_UNLOCK();
    zlog(
      g_zc,
  ...
  while ( 1 )
  {
    ...
    if ( v0 < 0.0 )
    {
      V_LOCK();
      ++v2;
      logfmt_raw(v16, 0x1000u, 0, 1342384);
      V_UNLOCK();
      v8 = g_zc;
      v9 = 100;
      v10 = 89;
LABEL_13:
    ...
    if ( v0 * 100.0 > (double)current_voltage * 1.1 || v6 < v5 * 0.9 )
    {
      ...
      if ( v6 - v5 <= 0.0 )
        v7 = "drop";
      logfmt_raw(v16, 0x1000u, 0, 1342432, v7, current_voltage, v0 * 100.0);
      V_UNLOCK();
      v8 = g_zc;
      v9 = 80;
      v10 = 93;
...
LABEL_14:
    V_LOCK();
    logfmt_raw(v16, 0x1000u, 0, 1342484, current_voltage, v15, v0 * 100.0);
    V_UNLOCK();
    zlog(
      g_zc,
      "/workspace/jenkins/jenkins/workspace/Antminer_D9_release/build/rootfs/buildroot/tmp/release/build/godminer-origin_"
...
LABEL_4:
  ...
  if ( v2 <= 3 )
    goto LABEL_4;
  V_LOCK();
  logfmt_raw(v16, 0x1000u, 0, 1342572);
  V_UNLOCK();
  return zlog(
           g_zc,

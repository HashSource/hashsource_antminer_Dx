void __noreturn nonce_submit_thread()
{
  ...
  V_UNLOCK();
  LOWORD(v5) = 4264;
  HIWORD(v5) = (unsigned int)"or: jobid %p cancel %d" >> 16;
  zlog(g_zc, v5, 138, "nonce_submit_thread", 19, 306, 40, v24);
  prctl(15, v2);
  v6 = pthread_self();
  pthread_detach(v6);
  ...
  LOWORD(v8) = 5168;
  LOWORD(v9) = 4264;
  HIWORD(v8) = (unsigned int)"er version file %s error" >> 16;
  HIWORD(v9) = (unsigned int)"or: jobid %p cancel %d" >> 16;
  v19 = v8;
  v20 = v9;
  while ( 1 )
  {
    while ( 1 )
    {
      ...
      V_UNLOCK();
      zlog(g_zc, v20, 138, "nonce_submit_thread", 19, 321, 80, v24);
    }

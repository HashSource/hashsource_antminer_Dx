int check_and_destroy_abnormal_runtime()
{
  ...
  if ( dword_1A3CB0 > 0 )
  {
    ...
    do
    {
      ...
      {
        V_LOCK();
        logfmt_raw(v10, 0x1000u, 0, "chain %d runtime check asic number error, destroy it!", *((_DWORD *)*v2 + 64));
        V_UNLOCK();
        zlog(
          g_zc,
          ...
          80,
          v10);
        set_miner_6060info_asic_num_less_than_design_err(*((_DWORD *)*v2 + 64), 1);
        (*((void (**)(void))*v2 + 6))();
        (*((void (**)(void))*v2 + 8))();
        free(*v2);
        *v2 = 0;

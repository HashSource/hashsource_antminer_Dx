int __fastcall eeprom_read(unsigned int a1, unsigned __int8 a2, int a3, int a4)
{
  ...
  {
    v13 = -2147483391;
    snprintf((char *)s, 0x800u, "%s: Bad eeprom param, input chain is %d", "eeprom_read", a1);
    V_LOCK();
  ...
  v5 = 8 * a1;
  if ( !*(_DWORD *)&algn_1B4540[8 * a1 + 68] )
  {
    v13 = eeprom_open(a1);
    if ( v13 < 0 )
    {
      snprintf((char *)s, 0x800u, "%s: auto exec eeprom_open, but chain %d open eeprom failed", "eeprom_read", a1);
      V_LOCK();
      logfmt_raw(v18, 0x1000u, 0, s);
      V_UNLOCK();
      zlog(
  ...
  if ( a4 )
  {
    ...
    v13 = -2147483392;
    snprintf((char *)s, 0x800u, "fail to read eeprom by iic, chain: %d, addr: %d", a1, v16);
    V_LOCK();
    logfmt_raw(v18, 0x1000u, 0, s);
    V_UNLOCK();
    zlog(

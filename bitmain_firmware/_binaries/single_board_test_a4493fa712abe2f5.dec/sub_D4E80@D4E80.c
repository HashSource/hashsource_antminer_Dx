int __fastcall sub_D4E80(int a1, int a2, int a3, _BYTE *a4)
{
  int v5; // r6
  int v6; // r4
  int v7; // r11
  ...
  *(_BYTE *)(a3 + 5) = 44;
  while ( 1 )
  {
    ...
    {
      i2c_read(v8, (int)a4, 13);
      pthread_mutex_unlock(&stru_3B5254);
    }
    pthread_mutex_unlock(&i2c_mutex_all);
    if ( a4[1] == 40 && *a4 == 13 )
      break;
    snprintf(s, 0x800u, "PICCMD %d failed on Chain[%d]!", 40, v5);
    pthread_mutex_lock(&stru_3B526C);
    logfmt_raw(v12, 0x1000u, 0, s);
    pthread_mutex_unlock(&stru_3B526C);
    zlog(
  ...
  snprintf(s, 0x800u, "PICCMD %d passed on Chain[%d]!", 40, v5);
  V_LOCK();
  logfmt_raw(v12, 0x1000u, 0, s);
  V_UNLOCK();
  zlog(

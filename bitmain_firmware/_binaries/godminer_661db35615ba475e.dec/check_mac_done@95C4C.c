int __fastcall check_mac_done(int a1, int a2, int a3, int a4)
{
  unsigned __int8 v5; // r7
  int v7; // r0
  int v8; // r1
  int v9; // r2
  int *v10; // r12
  unsigned __int8 *v11; // r5
  __int16 v12; // r3
  int v13; // r3
  int v14; // r8
  int v15; // r0
  _DWORD *v16; // r4
  int v17; // r0
  int v18; // r2
  bool v19; // zf
  bool v20; // zf
  int v21; // r3
  int v22; // r7
  int v23; // r0
  int v24; // r12
  int v25; // r2
  _BOOL4 v27; // r3
  int v28; // [sp+14h] [bp-1030h]
  int v29; // [sp+18h] [bp-102Ch]
  int v30; // [sp+20h] [bp-1024h]
  int v32; // [sp+28h] [bp-101Ch]
  _DWORD v33[4]; // [sp+30h] [bp-1014h] BYREF
  char v34[4100]; // [sp+40h] [bp-1004h] BYREF

  v5 = a3;
  pthread_mutex_lock(&stru_197BB8);
  logfmt_raw(v34, 0x1000u, 0, "%s...", "check_mac_done");
  pthread_mutex_unlock(&stru_197BB8);
  zlog(
    g_zc,
    "/workspace/jenkins/jenkins/workspace/Antminer_D7_release/build/rootfs/buildroot/tmp/release/build/godminer-origin_ma"
    "ster/backend/backend_eth/backend_eth.c",
    154,
    "check_mac_done",
    14,
    427,
    60,
    v34);
  sub_8F7C8(a1, v5, 149, 0);
  sub_8F7C8(a1, v5, 151, 0);
  sub_8F7C8(a1, v5, 153, 0);
  sub_8F7C8(a1, v5, 87, 255);
  sub_8F7C8(a1, v5, 159, 0);
  sub_8F7C8(a1, v5, 133, 805306367);
  if ( a4 )
  {
    if ( a4 == 1 )
    {
      sub_8F7C8(a1, a3, 134, 0xFFFFFFF);
      sub_8F7C8(a1, a3, 132, -2147483647);
    }
  }
  else
  {
    sub_8F7C8(a1, a3, 134, 0xFFFFFFF);
    sub_8F7C8(a1, a3, 132, 17);
  }
  sleep(8u);
  v7 = (int)malloc(0x600u);
  v11 = (unsigned __int8 *)v7;
  v33[1] = 0;
  if ( a3 == 255 )
    v9 = 14;
  v33[0] = 0;
  if ( a3 == 255 )
    v10 = v33;
  v33[2] = 0;
  v33[3] = 0;
  if ( a3 == 255 )
  {
    v32 = v9;
    v7 = 100991489;
    v8 = 235735561;
    v9 = 134677248;
    v12 = 3851;
  }
  else
  {
    v12 = 1;
    v32 = 1;
  }
  if ( a3 == 255 )
  {
    *v10 = v7;
    v10[1] = v8;
    v10[2] = v9;
    *((_WORD *)v10 + 6) = v12;
  }
  v30 = 0;
  do
  {
    v13 = a3;
    if ( a3 == 255 )
      v13 = *((unsigned __int8 *)v33 + v30);
    v28 = v13;
    v14 = (unsigned __int16)v13;
    ((void (__fastcall *)(int))off_18F508[0])(*(_DWORD *)(a1 + 120));
    v15 = sub_93DE0(a1, v14, 139, v11);
    if ( v15 > 0 )
    {
      v16 = v11 + 16;
      v29 = (int)&v11[12 * v15 + 16];
      while ( 1 )
      {
        while ( 1 )
        {
          v21 = *(v16 - 4);
          __pld(v16);
          v22 = *((unsigned __int8 *)v16 - 12);
          if ( (v21 & 0xFF000000) == 0xFF000000 )
            break;
          __pld(v16 - 1);
          pthread_mutex_lock(&stru_197BB8);
          logfmt_raw(
            v34,
            0x1000u,
            0,
            "chain %d xxxxxxxxxx chip %02x core %02x mac bist not done(%08x) xxxxxxxxxxx",
            *(_DWORD *)(a1 + 120),
            v22,
            *((unsigned __int8 *)v16 - 8),
            bswap32(*(v16 - 4)));
          pthread_mutex_unlock(&stru_197BB8);
          v23 = g_zc;
          v24 = 60;
          v25 = 532;
LABEL_40:
          v16 += 3;
          zlog(
            v23,
            "/workspace/jenkins/jenkins/workspace/Antminer_D7_release/build/rootfs/buildroot/tmp/release/build/godminer-o"
            "rigin_master/backend/backend_eth/backend_eth.c",
            154,
            "check_mac_done",
            14,
            v25,
            v24,
            v34);
          if ( (_DWORD *)v29 == v16 )
            goto LABEL_41;
        }
        if ( (unsigned __int8)v21 == 255 )
          break;
        if ( v28 == 3 )
        {
          if ( (*(v16 - 4) & 0xF0) == 0xF0 )
          {
            sub_9577C(a1, v22, 3, 135, v11);
            goto LABEL_27;
          }
LABEL_47:
          pthread_mutex_lock(&stru_197BB8);
          logfmt_raw(
            v34,
            0x1000u,
            0,
            "chain %d chip %02x core %02x mac bist status %02x",
            *(_DWORD *)(a1 + 120),
            *((unsigned __int8 *)v16 - 12),
            *((unsigned __int8 *)v16 - 8),
            *(v16 - 4) & 0xF0);
          pthread_mutex_unlock(&stru_197BB8);
          v17 = g_zc;
          v18 = 491;
          goto LABEL_21;
        }
        pthread_mutex_lock(&stru_197BB8);
        logfmt_raw(
          v34,
          0x1000u,
          0,
          "chain %d chip %02x core %02x mac bist status %02x",
          *(_DWORD *)(a1 + 120),
          *((unsigned __int8 *)v16 - 12),
          *((unsigned __int8 *)v16 - 8),
          *((unsigned __int8 *)v16 - 16));
        pthread_mutex_unlock(&stru_197BB8);
        v17 = g_zc;
        v18 = 488;
LABEL_21:
        zlog(
          v17,
          "/workspace/jenkins/jenkins/workspace/Antminer_D7_release/build/rootfs/buildroot/tmp/release/build/godminer-ori"
          "gin_master/backend/backend_eth/backend_eth.c",
          154,
          "check_mac_done",
          14,
          v18,
          100,
          v34);
LABEL_22:
        if ( sub_9577C(a1, v22, v14, 135, v11) )
        {
          v19 = *(_DWORD *)v11 == 0;
          if ( *(_DWORD *)v11 )
            v19 = v28 == 3;
          if ( !v19 )
          {
            pthread_mutex_lock(&stru_197BB8);
            logfmt_raw(
              v34,
              0x1000u,
              0,
              "chain %d channel[0~3] chip %02x core %02x reg 0x87 mac bist err cnt %08x",
              *(_DWORD *)(a1 + 120),
              v11[4],
              v11[8],
              bswap32(*(_DWORD *)v11));
            pthread_mutex_unlock(&stru_197BB8);
            zlog(
              g_zc,
              "/workspace/jenkins/jenkins/workspace/Antminer_D7_release/build/rootfs/buildroot/tmp/release/build/godminer"
              "-origin_master/backend/backend_eth/backend_eth.c",
              154,
              "check_mac_done",
              14,
              497,
              100,
              v34);
          }
        }
LABEL_27:
        if ( sub_9577C(a1, v22, v14, 136, v11) && *(_DWORD *)v11 )
        {
          pthread_mutex_lock(&stru_197BB8);
          logfmt_raw(
            v34,
            0x1000u,
            0,
            "chain %d channel[4~7] chip %02x core %02x reg 0x88 mac bist err cnt %08x",
            *(_DWORD *)(a1 + 120),
            v11[4],
            v11[8],
            bswap32(*(_DWORD *)v11));
          pthread_mutex_unlock(&stru_197BB8);
          zlog(
            g_zc,
            "/workspace/jenkins/jenkins/workspace/Antminer_D7_release/build/rootfs/buildroot/tmp/release/build/godminer-o"
            "rigin_master/backend/backend_eth/backend_eth.c",
            154,
            "check_mac_done",
            14,
            506,
            100,
            v34);
        }
        if ( sub_9577C(a1, v22, v14, 137, v11) )
        {
          v20 = *(_DWORD *)v11 == 0;
          if ( *(_DWORD *)v11 )
            v20 = v28 == 3;
          if ( !v20 )
          {
            pthread_mutex_lock(&stru_197BB8);
            logfmt_raw(
              v34,
              0x1000u,
              0,
              "chain %d channel[0~3] chip %02x core %02x reg 0x89 mac bist lost cnt %08x",
              *(_DWORD *)(a1 + 120),
              v11[4],
              v11[8],
              bswap32(*(_DWORD *)v11));
            pthread_mutex_unlock(&stru_197BB8);
            zlog(
              g_zc,
              "/workspace/jenkins/jenkins/workspace/Antminer_D7_release/build/rootfs/buildroot/tmp/release/build/godminer"
              "-origin_master/backend/backend_eth/backend_eth.c",
              154,
              "check_mac_done",
              14,
              515,
              100,
              v34);
          }
        }
        if ( sub_9577C(a1, v22, v14, 138, v11) && *(_DWORD *)v11 )
        {
          pthread_mutex_lock(&stru_197BB8);
          logfmt_raw(
            v34,
            0x1000u,
            0,
            "chain %d channel[4~7] chip %02x core %02x reg 0x8a mac bist lost cnt %08x",
            *(_DWORD *)(a1 + 120),
            v11[4],
            v11[8],
            bswap32(*(_DWORD *)v11));
          pthread_mutex_unlock(&stru_197BB8);
          v23 = g_zc;
          v24 = 100;
          v25 = 524;
          goto LABEL_40;
        }
        v16 += 3;
        if ( (_DWORD *)v29 == v16 )
          goto LABEL_41;
      }
      v27 = (*(v16 - 4) & 0xF0) != 240;
      if ( v28 != 3 )
        v27 = 0;
      if ( !v27 )
        goto LABEL_22;
      goto LABEL_47;
    }
LABEL_41:
    v19 = v32 == ++v30;
  }
  while ( !v19 );
  free(v11);
  return 0;
}

int __fastcall sub_135B68(int a1, int a2, int a3, _BYTE *a4)
{
  int v5; // r5
  int v6; // r11
  int v7; // r11
  ...
  *(_BYTE *)(a3 + 5) = 26;
  while ( 1 )
  {
    ...
    {
      i2c_read(v7, (int)a4, 6);
      pthread_mutex_unlock(&stru_197C58);
    }
    pthread_mutex_unlock(&i2c_mutex_all);
    if ( a4[1] == 22 && *a4 == 6 )
      break;
    snprintf(v12, 0x800u, "PICCMD %d failed on Chain[%d]!", 22, v5);
    pthread_mutex_lock(&stru_197BB8);
    logfmt_raw(v13, 0x1000u, 0, v12);
    pthread_mutex_unlock(&stru_197BB8);
    zlog(
  ...
  snprintf(v12, 0x800u, "PICCMD %d passed on Chain[%d]!", 22, v5);
  V_LOCK();
  logfmt_raw(v13, 0x1000u, 0, v12);
  V_UNLOCK();
  zlog(
